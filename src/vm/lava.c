...
static void wait_no_key()
{
	...
	for (;;) {
		...
		if (bbk9588_platform_should_exit()) break;
		t=0;
...
{
#if NO_RAM_ALIGN
	return lRam[addr]|(lRam[addr+1]<<8);
#else
...
{
#if NO_RAM_ALIGN
	return lRam[addr]|(lRam[addr+1]<<8)|(lRam[addr+2]<<16)|(lRam[addr+3]<<24);
#else
...
void c_strcmp()
{
	...
	t=strcmp((char*)lRam+t1,(char*)lRam+t2);
	if (t==0) a1=0;
	else if (t>0) a1=1;
	else a1=-1;
	put_val();
...
void c_strstr()
{
	...
	p=(byte*)strstr((char*)lRam+t1,(char*)lRam+t2);
	if (p==NULL) a1=0;
	else a1=p-lRam;
	put_val();
}
...
void c_getchar()
{
	while (lav_key<128 && !bbk9588_platform_should_exit()) {
		bbk9588_platform_tick();
	}
	if (lav_key<128) lav_key=0x80|0x1b;
	a1=lav_key&0x7f;
	lav_key&=0x7f;
	put_val();
}
...
void c_exec()
{
	...
	strcpy((char*)lRam+local_sp+CMDLINE,VirtualFullName);
	strcat((char*)lRam+local_sp+CMDLINE," ");
	strcat((char*)lRam+local_sp+CMDLINE,(char*)lRam+a3);
...
void c_system()
{
	...
	case 32: //PeekMessage
		...
		if (RamBits>16 && lRam[a1]==1) {
			GetTouch((struct TOUCH*)(lRam+a1));
			a1=-1;
		} else {
			a1=0;
		}
